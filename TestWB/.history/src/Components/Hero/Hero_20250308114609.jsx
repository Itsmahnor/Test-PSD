...
export const Hero = () => {
  ...
  const settings = {
    ...
    dots: true,
    appendDots: (dots) => (
      <div className="absolute bottom-[45%] w-full flex justify-center">
        <ul className="flex gap-3">{dots}</ul>
      </div>
    ),
    customPaging: () => (
      <div className="w-16 h-1 bg-green-600 opacity-50 hover:opacity-100 transition-all cursor-pointer" />
    ),
  };
        ...
        {slides.map((slide) => (
          <div key={slide.id} className="relative w-full h-[70vh]">
            ...
            <div className="relative flex flex-col-reverse md:flex-row items-center justify-between px-6 lg:px-36 py-12 w-full h-full text-white">
              {/* Dots (Navigation Bars) in Between */}
              <div className="absolute bottom-[45%] w-full flex justify-center z-10">
                <ul className="flex gap-3">
                  {slides.map((_, index) => (
                    <li key={index} className="w-16 h-1 bg-white opacity-50 hover:opacity-100 transition-all cursor-pointer" />
                  ))}
                </ul>
              </div>
