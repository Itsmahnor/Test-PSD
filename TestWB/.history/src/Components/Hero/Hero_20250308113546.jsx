...
export const Hero = () => {
  ...
  const settings = {
      ...
      <div className="absolute bottom-4 w-full flex justify-center z-20">
        ...
        <ul className="flex gap-3">
          {dots.map((dot, index) => (
            <li
              key={index}
              className="w-10 h-1 bg-white rounded cursor-pointer transition-all duration-300 hover:bg-gray-300"
            >
              {dot}
            </li>
          ))}
    ...
    customPaging: () => <div className="w-full h-full" />,
  };
  ...
  return (
    <div className="w-full h-[70vh]"> {/* Set height to 70% of viewport */}
      <Slider {...settings}>
        {slides.map((slide) => (
          <div key={slide.id} className="relative w-full h-[70vh]"> {/* Adjust height */}
            {/* Background Image */}
            <div
            ...
            <div className="relative flex flex-col-reverse md:flex-row items-center justify-between px-6 lg:px-36 py-12 w-full h-full text-white">
              {/* Right Side: Thumbnail with Play Icon (Now on top for small screens) */}
              ...
              {/* Left Side: Text (Now below video for small screens) */}
